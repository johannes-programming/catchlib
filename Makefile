...
test: beautiful dist
	conda run -n base python make/env.py test_catchlib --python=3.11 --recreate >/dev/null;
	conda run -n test_catchlib pip install dist/*.tar.gz >/dev/null;
	conda run -n test_catchlib python run_tests.py;
	conda run -n test_catchlib pip install mypy >/dev/null;
	conda run -n test_catchlib python -m mypy --exclude build --exclude dist --strict .;
	conda run -n test_catchlib python -m mypy --strict -p catchlib;
